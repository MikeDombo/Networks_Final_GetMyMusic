...
uint32_t checksumInternals(const char* inputBuffer, size_t inputSize) {
    // Wikipedia: algorithm is
    /*
        crc32 ← 0xffffffff
    ...
    uint8_t nLookupIndex;
    for (size_t i = 0; i < inputSize; ++i) {
        nLookupIndex = static_cast<u_int8_t>((crc32 ^ static_cast<uint8_t>(inputBuffer[i])) & 0xff);
...
int main() {
    ...
    cout << "  Performing division by a \"binary polynomial\"" << endl;
    int msg = 0x0617;
    int poly = 9;
    int remainder = msg % poly;
    assert(remainder == 2);

    char test1[] = "12345678901234567890";
    uint32_t res1 = checksumInternals(test1, strlen(test1));
